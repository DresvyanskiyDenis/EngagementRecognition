...
def get_modified_VGGFace2_resnet_model(dense_neurons_after_conv: Tuple[int,...],
    ...
    if multi_head_attention:
        for _ in num_classes:
            ...
            class_x = tf.keras.layers.AveragePooling2D()(class_x)

            for dense_layer_idx in range(len(dense_neurons_after_conv) - 1):
                num_neurons_on_layer = dense_neurons_after_conv[dense_layer_idx]
                class_x = tf.keras.layers.Dense(num_neurons_on_layer, activation='relu', kernel_regularizer=regularization)(class_x)
                if dropout:
...
if __name__ == '__main__':
    ...
    labels_train.loc[(labels_train['engagement_0'] != 0),'engagement_0']=0
    labels_train.loc[(labels_train['engagement_0'] == 0), 'engagement_0'] = 1
