#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Contains the functions for the IWSDS2023 labels preprocessing.
...
def read_noxi_label_file(path: str) -> np.ndarray:
    """Reads the label file of IWSDS2023 database
...
def average_from_several_labels(labels_list: List[np.ndarray]) -> np.ndarray:
    """Averages labels from different sources (annotaters) into one labels file.
       In IWSDS2023, aprat from the labels itself there are confidences of the labeling.
