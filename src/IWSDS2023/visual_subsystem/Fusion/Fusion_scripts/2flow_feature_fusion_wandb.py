...
def preprocess_and_align_data(df1, df2):
...
def load_data(scaler:str):
    ...
    train_generator = Nflow_FusionDataLoader((train_generator_flow_1, train_generator_flow_2))
    dev_generator = Nflow_FusionDataLoader((dev_generator_flow_1, dev_generator_flow_2))
...
def train_model(train:torch.utils.data.DataLoader, dev:torch.utils.data.DataLoader,
    ...
    metaparams = {
        ...
        "epochs": 100,
        "batch_size": 256,
        "architecture": "2Flow_fusion_network",
        "dataset": "IWSDS2023",
        "num_classes": 5,
        "input_shapes": [data_shape[-1], data_shape[-1]],
...
def run(scaler:str):
    print("New start...!")
    sweep_config = {
        'name': "2Flow_fusion_f2f_%sScaler_focal_loss" % scaler,
        'method': 'random',
        'metric': {
            'name': 'val_loss',
            'goal': 'minimize'
