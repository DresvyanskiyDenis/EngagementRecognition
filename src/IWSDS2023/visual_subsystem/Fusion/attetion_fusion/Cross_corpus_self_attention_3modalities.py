...
def load_data(language:str, window_length:int, window_shift:int):
    ...
    train_3 = pd.read_csv("/work/home/dsu/IWSDS2023/NoXi_embeddings/Cross-corpus/%s/"
    ...
    train_3['filename'] = train_3['filename'].apply(lambda x: os.path.join(*x.split(os.path.sep)[-3:]))
    ...
    dev_3 = pd.read_csv("/work/home/dsu/IWSDS2023/NoXi_embeddings/Cross-corpus/%s/"
    ...
    dev_3['filename'] = dev_3['filename'].apply(lambda x: os.path.join(*x.split(os.path.sep)[-3:]))

    # create generators
    train_generator = Nflow_FusionSequenceDataLoader(embeddings=[train_1, train_2, train_3], window_length=window_length,
                                                     window_shift=window_shift,
                                                     scalers=["standard", "standard", "standard"], labels_included=True,
    ...
    dev_generator = Nflow_FusionSequenceDataLoader(embeddings=[dev_1, dev_2, dev_3], window_length=window_length,
                                                   window_shift=window_shift,
                                                   scalers=[scalers[0], scalers[1], scalers[2]], labels_included=True,
                                                   sequence_to_one=True, data_as_list=True)
...
def run(window_length:int, language:str):
    print("New start...!!!")
    sweep_config = {
        'name': "self_attention_3Flow_all_vs_%s_window_%i" % (language, window_length),
        'method': 'random',
        'metric': {
            'name': 'val_loss',
            'goal': 'minimize'
