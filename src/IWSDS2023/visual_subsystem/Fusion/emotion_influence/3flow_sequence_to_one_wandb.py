...
def load_data(window_length:int, window_shift:int, scaler:str='PCA'):
    ...
    train_3['filename'] = train_3['filename'].apply(lambda x: os.path.join(*x.split("/")[-3:]))
    ...
    dev_3['filename'] = dev_3['filename'].apply(lambda x: os.path.join(*x.split("/")[-3:]))
    ...
    if scaler == "PCA":
        ...
        PCA_components = None
    # create generators
    train_generator = Nflow_FusionSequenceDataLoader(embeddings=[train_1, train_2, train_3], window_length=window_length,
                                                     window_shift=window_shift,
                                                     scalers=['PCA', 'PCA', 'PCA'], labels_included=True, PCA_components=PCA_components,
                                                     sequence_to_one=True)
    scalers = train_generator.scalers
    dev_generator = Nflow_FusionSequenceDataLoader(embeddings=[dev_1, dev_2, dev_3], window_length=window_length,
                                                   window_shift=window_shift,
                                                   scalers=[scalers[0], scalers[1], scalers[2]], labels_included=True, PCA_components=PCA_components,
                                                   sequence_to_one=True)
...
def run(window_length:int):
    print("New start...!!!")
    sweep_config = {
        'name': "3Flow_fusion_Seq2One_PCA_window_%i" % window_length,
        'method': 'random',
        'metric': {
            'name': 'val_loss',
            'goal': 'minimize'
