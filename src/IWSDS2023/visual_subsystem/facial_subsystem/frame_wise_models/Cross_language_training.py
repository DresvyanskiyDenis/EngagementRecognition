#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Contains the script for training the Xception model on the IWSDS2023 dataset. The Weights and Biases library is used to
...
from src.IWSDS2023.visual_subsystem.facial_subsystem.frame_wise_models.utils import load_NoXi_data_cross_corpus
...
def train_model(train, dev, loss_func='categorical_crossentropy'):
    ...
    for i, layer in enumerate(model.layers):
        print("%i:%s, trainable:%s" % (i, layer.name, layer.trainable))
    ...
    dev_data_loader = get_tensorflow_image_loader(paths_and_labels=dev, batch_size=80,
                                               augmentation=False,
