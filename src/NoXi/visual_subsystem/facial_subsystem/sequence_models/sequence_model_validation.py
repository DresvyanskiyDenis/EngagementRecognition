#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Contains the script for validation and test the sequence model (tf.keras.Model) on the NoXi data.
...
def main():
    ...
    for index, row in params_for_testing_models.iterrows():
        dev_res, test_res = validate_model(dev, test, dir_with_models + row["weights_path"], row["window_length"],
                                           row["num_neurons"], row["num_layers"])
