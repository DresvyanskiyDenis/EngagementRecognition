...
def train_model(train, dev, test, epochs:int, class_weights:Optional=None, loss_function:str="Crossentropy"):
    ...
    wandb.init(project="NoXi_FtF_training", config=metaparams)
    config = wandb.config
    ...
    print('Metaparams: optimizer: %s, learning_rate:%f, lr_scheduller: %s, annealing_period: %d, epochs: %d, '
          ...
          'dataset: %s, num_classes: %d' % (config.optimizer, config.learning_rate_max, config.lr_scheduller,
                                            config.annealing_period, epochs, config.batch_size,
...
def main(language:str):
    print("Start...")
    sweep_config = {
        'name': "HRNet_f2f_Focal_loss_cross_corpus_%s_vs_all"%language,
        'method': 'random',
        'metric': {
            'name': 'val_loss',
            'goal': 'minimize'
    ...
    sweep_id = wandb.sweep(sweep_config, project='NoXi_FtF_training')
