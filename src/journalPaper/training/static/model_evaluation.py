...
def evaluate_model(model: torch.nn.Module, generator: torch.utils.data.DataLoader, device: torch.device,
    ...
    with torch.no_grad():
        for i, data in enumerate(generator):
            ...
            classification_output = np.argmax(classification_output, axis=-1, keepdims=True) if len(classification_output.shape) == 1 \
            ...
            classification_ground_truth = np.argmax(classification_ground_truth, axis=-1, keepdims=True) if len(classification_ground_truth.shape) == 1 \
